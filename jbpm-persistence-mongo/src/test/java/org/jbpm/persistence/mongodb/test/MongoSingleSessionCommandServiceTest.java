...
public class MongoSingleSessionCommandServiceTest extends AbstractMongoBaseTest {
    ...
    @Test
    public void testPersistenceSubProcess() throws ClassNotFoundException, IOException {
    	SessionConfiguration config = initSessionConfig();

        RuleBase ruleBase = RuleBaseFactory.newRuleBase();
        Package pkg = getProcessSubProcess();
        ruleBase.addPackage( pkg );

        MongoSingleSessionCommandService service = new MongoSingleSessionCommandService( ruleBase,
                                                                               config,
                                                                               getEnv() );
        int sessionId = service.getSessionId();
        StartProcessCommand startProcessCommand = new StartProcessCommand();
    ...
    public void testPersistenceTimer() throws Exception {
        ...
        ProcessInstance processInstance = service.execute( startProcessCommand );
        logger.info( "Started process instance {}", processInstance.getId() );
        TimerManager timerManager = ((InternalProcessRuntime)((StatefulKnowledgeSessionImpl)service.getKieSession()).getInternalWorkingMemory().getProcessRuntime()).getTimerManager();
        assertTrue(timerManager.getTimerMap().size() > 0);
        TimerInstance ti = (TimerInstance)timerManager.getTimerMap().values().toArray()[0];
        System.out.println("processInstanceId in timer:" + ti.getProcessInstanceId());
        System.out.println("delay in timer:" + ti.getDelay());
        assertTrue(ti.getSessionId()== sessionId);
        ...
        processInstance = service.execute( getProcessInstanceCommand );
        assertNotNull( processInstance );
        timerManager = ((InternalProcessRuntime)((StatefulKnowledgeSessionImpl)service.getKieSession()).getInternalWorkingMemory().getProcessRuntime()).getTimerManager();
        assertTrue(timerManager.getTimerMap().size() > 0);
        ti = (TimerInstance)timerManager.getTimerMap().values().toArray()[0];
        System.out.println("processInstanceId in timer:" + ti.getProcessInstanceId());
        System.out.println("delay in timer:" + ti.getDelay());
        assertTrue(ti.getSessionId()== sessionId);
        service.dispose();
	...
	private SessionConfiguration initSessionConfig() {
		Properties properties = new Properties();
        properties.setProperty( "drools.commandService",
                                MongoSingleSessionCommandService.class.getName() );
        properties.setProperty( "drools.processInstanceManagerFactory",
